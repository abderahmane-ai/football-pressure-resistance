import numpy as np
import pandas as pd
from .geometry import voronoi_area, angular_span, pitch_control_value, xt_value, lane_unblocked
...
def extract_spatial_features_from_frame(frame_data, ball_carrier_player_id, team_id, opponent_team_id, match_context=None):
    ...
    coverage_radius = SPATIAL_CONFIG['coverage_arc_radius']
    
    if len(opps) > 0:
        opp_dists = np.linalg.norm(opps - bc, axis=1)
        sorted_opp_dists = np.sort(opp_dists)
        features['dist_nearest_opp'] = sorted_opp_dists[0]
        features['dist_2nd_nearest_opp'] = sorted_opp_dists[1] if len(sorted_opp_dists) > 1 else pitch_len
