...
def run_interpretability_analysis() -> None:
    ...
    if not p.trace.exists():
        logger.warning("Trace not found for interpretability: %s", p.trace)
        return

    ctx = load_posterior_context(holdout)
    ...
    global_names = [feature_names[i] for i in range(len(feature_names)) if ctx.global_mask[i]]
    pos_names = [feature_names[i] for i in range(len(feature_names)) if ctx.pos_specific_mask[i]]

    # Aggregate position-specific β to mean across positions
    pos_mean_succ = ctx.beta_pos_succ.mean(axis=1)  # (n_samples, ctx.n_pos_specific)
    pos_mean_val = ctx.beta_pos_val.mean(axis=1)

    # Combine global + mean position-specific for a single feature importance table
    combined_beta_succ = np.column_stack([ctx.beta_global_succ, pos_mean_succ])
    combined_beta_val = np.column_stack([ctx.beta_global_val, pos_mean_val])
    ...
    for p_code, pos_name in pos_mapping.items():
        for j, f_name in enumerate(pos_names):
            for model_label, arr in [("ball_security", ctx.beta_pos_succ), ("value_retention", ctx.beta_pos_val)]:
    ...
    # Dataset is already loaded and spline-expanded in ctx.df
    ...
    theta_chol_flat = ctx.theta_chol_packed.reshape(-1, ctx.theta_chol_packed.shape[-1]) if ctx.theta_chol_packed is not None else np.zeros((1, 3))
    sigma_theta_succ = theta_chol_flat[:, 0]
    var_theta_succ = float(np.mean(sigma_theta_succ**2))

    sigma_opp_succ_arr = ctx.sigma_opp_succ if ctx.sigma_opp_succ is not None else np.array([0.0])
    var_opp_succ = float(np.mean(sigma_opp_succ_arr**2))

    sigma_comp_succ_arr = ctx.sigma_comp_succ if ctx.sigma_comp_succ is not None else np.array([0.0])
    var_comp_succ = float(np.mean(sigma_comp_succ_arr**2))

    sigma_team_succ_arr = ctx.sigma_team_succ if ctx.sigma_team_succ is not None else np.array([0.0])
    var_team_succ = float(np.mean(sigma_team_succ_arr**2))

    X_global = X_scaled[:, ctx.global_mask]
    X_pos = X_scaled[:, ctx.pos_specific_mask]
    n_samples_succ = ctx.beta_global_succ.shape[0]
    global_contrib = np.dot(X_global, ctx.beta_global_succ.T)  # (n_obs, n_samples)
    pos_contrib = np.zeros_like(global_contrib)
    for s in range(n_samples_succ):
        pos_beta_at_pos = ctx.beta_pos_succ[s, pos_idx]  # (n_obs, ctx.n_pos_specific)
    ...
    sigma_opp_val_arr = ctx.sigma_opp_val if ctx.sigma_opp_val is not None else np.array([0.0])
    var_opp_val = float(np.mean(sigma_opp_val_arr**2))

    sigma_comp_val_arr = ctx.sigma_comp_val if ctx.sigma_comp_val is not None else np.array([0.0])
    var_comp_val = float(np.mean(sigma_comp_val_arr**2))

    sigma_team_val_arr = ctx.sigma_team_val if ctx.sigma_team_val is not None else np.array([0.0])
    var_team_val = float(np.mean(sigma_team_val_arr**2))

    n_samples_val = ctx.beta_global_val.shape[0]
    X_val_mask = ctx.df["success"].values.astype(bool)
    X_global_val = X_global[X_val_mask]
    X_pos_val = X_pos[X_val_mask]
    pos_idx_val = pos_idx[X_val_mask]
    global_contrib_val = np.dot(X_global_val, ctx.beta_global_val.T)
    pos_contrib_val = np.zeros_like(global_contrib_val)
    for s in range(n_samples_val):
        pos_beta_at_pos = ctx.beta_pos_val[s, pos_idx_val]
    ...
    def _feat_contrib(vec: np.ndarray, beta_g: np.ndarray, beta_p: np.ndarray, p_code: int) -> np.ndarray:
        ...
        return np.dot(beta_g, vec[ctx.global_mask]) + np.dot(beta_p[:, p_code], vec[ctx.pos_specific_mask])

    # ── 2b. Signal-to-Noise Ratio ─────────────────────────────────────────────
    theta_succ_samples = ctx.theta_succ
    y_success_labels = ctx.df["success"].values.astype(int)
    logit_succ_mean = (
        ctx.alpha_succ[:, np.newaxis]
        + np.column_stack([
            _feat_contrib(X_scaled[i], ctx.beta_global_succ, ctx.beta_pos_succ, pos_idx[i])
            for i in range(len(X_scaled))
        ])
        + ctx.gamma_pos_succ[:, pos_idx]
    )
    ...
    def compute_marginal(feat_idx: int, values_range: np.ndarray) -> pd.DataFrame:
        ...
        for val in values_range:
            ...
            feat_contrib = _feat_contrib(scenario_vec, ctx.beta_global_succ, ctx.beta_pos_succ, mid_pos_code)
            logit_succ = ctx.alpha_succ + feat_contrib + ctx.gamma_pos_succ[:, mid_pos_code]
            feat_contrib_val = _feat_contrib(scenario_vec, ctx.beta_global_val, ctx.beta_pos_val, mid_pos_code)
            logit_val = ctx.alpha_val + feat_contrib_val + ctx.gamma_pos_val[:, mid_pos_code]
    ...
    if dist_spline_cols and ctx.spline_transformers:
        spline_tr = ctx.spline_transformers.get('dist_nearest_opp')
        if spline_tr is not None:
            ...
            for j, val in enumerate(grid):
                ...
                for k in range(bases.shape[1]):
                    col_idx = ctx.name_to_idx[dist_spline_cols[k]]
                ...
                feat_contrib = _feat_contrib(vec, ctx.beta_global_succ, ctx.beta_pos_succ, mid_pos_code)
                logit_succ = ctx.alpha_succ + feat_contrib + ctx.gamma_pos_succ[:, mid_pos_code]
                feat_contrib_val = _feat_contrib(vec, ctx.beta_global_val, ctx.beta_pos_val, mid_pos_code)
                logit_val = ctx.alpha_val + feat_contrib_val + ctx.gamma_pos_val[:, mid_pos_code]
                probs_succ = expit(logit_succ)
                ev_val = expit(logit_val) * max_value + min_value
                total_ev = probs_succ * ev_val
        ...
        if dist_spline_cols and ctx.spline_transformers:
            ...
            if dist_tr is not None:
                ...
                for player in selected_players:
                    p_theta_succ = ctx.theta_succ[:, player['idx']]
                    p_theta_val = ctx.theta_val[:, player['idx']]
                    pc = player['pos_code']
                    p_pos_succ = ctx.gamma_pos_succ[:, pc]
                    p_pos_val = ctx.gamma_pos_val[:, pc]
                    ...
                    for j, dist_val in enumerate(dist_grid):
                        ...
                        for k in range(spline_bases.shape[1]):
                            ...
                            col_idx = ctx.name_to_idx[col_name]
                            scenario_vec[col_idx] = (spline_bases[j, k] - scaler.mean_[col_idx]) / scaler.scale_[col_idx]

                        feat_contrib = _feat_contrib(scenario_vec, ctx.beta_global_succ, ctx.beta_pos_succ, pc)
                        logit_succ = ctx.alpha_succ + feat_contrib + p_pos_succ + p_theta_succ
                        feat_contrib_val = _feat_contrib(scenario_vec, ctx.beta_global_val, ctx.beta_pos_val, pc)
                        logit_val = ctx.alpha_val + feat_contrib_val + p_pos_val + p_theta_val
